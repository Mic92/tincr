...
| **6 chunk 12 — RMODE_SWITCH, TCPOnly, residuals** | scoped | | The remaining stub clusters. `chunk-12-switch` (16): wire `route_mac.rs`, `learn_mac` gossip, `age_subnets` timer, `Subnet::Mac` ADD/DEL, TAP device. `chunk-10-local` (6): `choose_local_address`/`adapt_socket` LAN-direct. `chunk-11-perf` (4): the 18% gap — per-packet `Vec` reuse (the C arena pattern), `recvmmsg`, the `maybe_set_write_any` sweep. `chunk-12-{bind,http-proxy,tcp-fallback}` singles. `chunk-9-relay` (6): TTL `getsockname` — affects only the TIME_EXCEEDED-from-relay-hop case (we're rarely the hop). `chunk-never` (3): `try_harder` brute-decrypt, legacy crypto. |
...
| `protocol_misc.c` | 376 | ~90% | PING/PONG (chunk 8). UDP_INFO/MTU_INFO gates+handlers wired (`udp_info.rs`, chunk 11). The 7 send-gates as `should_send_* → bool`, receive as `→ enum Action`. PACKET parse-and-swallow (`463b9987` — cross-impl found we crashed on it). Left: PACKET routing for TCPOnly (`STUB(chunk-12-tcp-fallback)`). |
| `net_packet.c` | 1938 | ~70% | **The hot path.** Chunk 7: send/recv core. Chunk 9: PMTU/compression/`try_tx` chain. `send_sptps_data` relay decision tree (`:965-1056`). Chunk 12-prep: `choose_local_address`/`adapt_socket` (`local_addr.rs`, leaf), `broadcast_packet` target selection (`broadcast.rs`, leaf), `receive_tcppacket_sptps` ladder (`tcp_tunnel.rs`, leaf). Left: daemon WIRING for those three, `try_harder` (chunk-never), `choose_initial_maxmtu` `getsockopt(IP_MTU)` (deferred-forever — PMTU converges from `MTU=1518` without it), legacy crypto (`:800-960`, chunk-never). |
| `route.c` | 1176 | ~90% | Chunk 7: `route_ipv4`. Chunk 9: v6/ICMP/MSS/ARP/NDP/TTL. **C-is-WRONG #8** (`:344`). `route_mac` leaf (`52f6f348`). `learn_mac`/`age_subnets` STATE leaf (`mac_lease.rs`, `bc9f223b`). Left: `route_broadcast` + `learn_mac`/`age_subnets` daemon WIRING (chunk-12-switch serial), `fragment_ipv4` (niche). |
...
| `proxy.c` | 285 | ✅ | `socks.rs` (`d988b79f`) + wired (`e841d05e`). **C-is-WRONG #9** (`:201` size_t→u8). `tcplen` multiplexed: pre-SPTPS proxy reply (`read_n`) vs post-SPTPS PACKET (record). `socks5_proxy_roundtrip`: in-process RFC 1928 server. HTTP CONNECT → `STUB(chunk-12-http-proxy)`. |
...
| `proxy.py` | `Proxy = socks5/http/exec` | S2 | 10 | ✅ socks5+exec (`e841d05e` chunk 11+, `1367cfaf` chunk 10). `socks5_proxy_roundtrip`: in-process RFC 1928 server, byte-exact. `proxy_exec_roundtrip("cat")`. HTTP CONNECT → `STUB(chunk-12-http-proxy)`. |
...
**Post-chunk-12-prep**: 16 of 35 covered (`ns_ping`, `device_fd`, `security`, `splice`, `compression`, `basic`, `sptps_basic`, `net.py::tunnel_server`, `invite/join` ×3, `scripts`, `proxy` (⅔) + 2 partial). 9 CLI-only (tinc-tools). 6 blocked on chunk-12-serial. 4 deliberately-never (`legacy_protocol`, `algorithms`, `sandbox`, `device_multicast`). The next gaps: `address_cache.py` (restart-then-dial), `device_tap.py` (chunk-12-switch wire), HTTP proxy.
