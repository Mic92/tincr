...
| `protocol_misc.c` | 376 | ~80% | PING/PONG (chunk 8). UDP_INFO/MTU_INFO gates+handlers (`udp_info.rs`, `40db12af` — chunk 11-prep, NOT yet wired). The 7 send-gates as `should_send_* → bool`, receive as `→ enum Action`. Left: TCPPACKET receive (TCP-relay encap), `STUB(chunk-11)` wire-up. |
...
| `proxy.c` | 285 | ✅ | `socks.rs` (`d988b79f`). **C-is-WRONG #9** (`:201` size_t→u8). Byte-offset reads, not `#[repr(C)]`. NOT yet wired — needs conn state machine (`STUB(chunk-11-proxy)`). |
| `multicast_device.c` | 224 | defer | niche backend. |
| `autoconnect.c` | 197 | ✅ | `autoconnect.rs` (`a68dbdcb`). `decide() → AutoAction`. The all-node prng IS the backoff. Seeded `ChaCha8Rng` for tests. NOT yet wired (`STUB(chunk-11)`). |
