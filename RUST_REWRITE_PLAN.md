...
| `route.c` | 1176 | ~85% | Chunk 7: `route_ipv4` decision. Chunk 9: `route_ipv6`, `route_ipv4/6_unreachable` (`icmp.rs`), `clamp_mss` (`mss.rs`), `route_arp`/`route_neighborsol` (`neighbor.rs`), `do_decrement_ttl`, `ratelimit`. **C-is-WRONG #8** (`:344`). `route_mac` leaf (`52f6f348`, chunk 11+): `(RouteResult, LearnAction)` two-channel. Left: `route_broadcast` (MST walk), `learn_mac`/`age_subnets` daemon-side (chunk 12), `fragment_ipv4` (niche). |
...
**Post-chunk-10**: 13 of 35 covered (`ns_ping`, `device_fd`, `security`, `splice`, `compression`, `basic`, `sptps_basic`, `net.py::tunnel_server`, `invite/join` ×3 + 3 partial). 9 CLI-only (tinc-tools). 9 blocked on chunks 11/12. 4 are deliberately-never (`legacy_protocol`, `algorithms`, `sandbox`, `device_multicast`). The next gaps: `scripts.py` ordering+env, `proxy.py` (chunk-11-proxy state machine), `address_cache.py` (restart-then-dial).
