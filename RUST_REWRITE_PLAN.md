...
| `protocol_key.c` | 648 | ~40% | `send_req_key` (initiator), `req_key_ext_h` `case REQ_KEY` (responder), `ans_key_h` SPTPS branch. Left: `REQ_PUBKEY`/`ANS_PUBKEY` (on-the-fly key fetch, `:196-231`), `SPTPS_PACKET` (TCP-tunneled data, `:149-188`), relay (`:320-344,462-484`), legacy `send_ans_key` (chunk-never). |
...
|---|---|---|---|---|---|
| **S1: test-process-as-peer** | `tests/stop.rs` (13 tests) | n/a — test process IS the peer | No | <100ms | Per-record SPTPS dispatch correctness. Can hand-craft adversarial records (wrong-key, malformed ADD_EDGE, the `seen_request` dup) that two cooperating daemons never produce. |
| **S2: two-real-daemons, fake TUN** | `tests/two_daemons.rs` (3 tests) | `socketpair(SEQPACKET)` via `DeviceType=fd` | No | <200ms (5s for backoff test) | **Epoll wake-chain bugs.** The mio EPOLLET fall-through (chunk 6) was invisible to S1 (test-side does blocking reads). S2 has two real epoll loops. |
| **S3: bwrap netns, real TUN** | `tests/netns.rs` (1 test) | `/dev/net/tun` via `DeviceType=tun` | Yes | ~3s | `TUNSETIFF` ioctl, `IFF_NO_PI` framing, kernel-generated checksums/ICMP. Everything `linux.rs` does. The `--tmpfs /dev` trick (kernel `2ab8baf` userns ownership check) makes this no-root. |
...
|---|---|---|---|---|
| `basic.py` | start/tinc-up/stop | S1 | 8 (scripts) | `spawn_connect_stop` is the start/stop half; tinc-up needs `execute_script` |
| `ns_ping.py` | netns + TUN + ping | S3 | 7 | ✅ `real_tun_ping` is this test, minus `require_root()` |
| `device_fd.py` | `DeviceType=fd` round-trip | S2 | 7 | ✅ `first_packet_across_tunnel` is exactly this |
| `cmd_dump.py` | `dump nodes/edges/subnets/connections` formatting | S1 | 7 | partial — `dump connections`/`subnets`/`nodes`/`edges` arms exist; the test asserts on FORMAT (`check.is_in("10.96.96.1", out)`). Our `dump_nodes` 23-field row needs round-tripping through `tinc-cli`'s parser. |
| `security.py` | adversarial ID lines, tarpit timing, own-ID rejection | S1 | 4a | `peer_wrong_key_fails_sig` covers one case. The id-timeout/tarpit-tarpitted/own-ID/metakey-too-short cases are 8 separate `async fn`s — port verbatim, each is one TcpStream + timed read. **High value: these are the protocol's security boundary.** |
| `splice.py` | MITM relay attack — `splice` binary connects to both, proxies. Daemon must drop on SIG mismatch. | S1 | 4a | not covered. The C ships a `splice` binary just for this test (`src/splice.c`, 100 LOC). We don't need the binary — test process opens two TcpStreams, copies bytes between them. Asserts both daemons reach 1 node (didn't accept the spliced peer). |
...
| `scripts.py` | tinc-up/down, host-up/down, subnet-up/down — order + env vars | S2 | 8 | `STUB(chunk-8): execute_script` markers. The C test has a notification socket per script; we'll use the same trick (script writes to a unix socket, test reads order). |
| `net.py::test_tunnel_server` | `TunnelServer = yes` filters indirect ADD_EDGE — foo↔mid↔bar, foo sees 2 nodes not 3 | S2 | 9 | 3-daemon S2 test. Our `STUB(chunk-9): tunnelserver mode` markers. |
...
| `compression.py` | `Compression = N` per-level (LZO/zlib/LZ4) — netns + TCP-over-tunnel content compare | S3 | 9 | `STUB(chunk-9): compression`. Port lands when `flate2`/`lz4_flex` wiring does. |
...
**14 of 35 are blocked on chunks 8-10.** 6 are already covered. 9 are CLI-only (tinc-tools, separate from tincd). The high-value gaps RIGHT NOW: `security.py` (8 adversarial cases, all S1, all chunk-4a-viable) and `splice.py` (the MITM test, S1, chunk-4a-viable). Those should be the next test-only worktree.

### Three-node S2/S3: the relay path

Everything above is two-node. The relay path (`via != myself`, `to->nexthop->connection`, the SPTPS_PACKET-over-TCP encapsulation) needs THREE: foo↔mid↔bar where foo and bar have no direct edge. `STUB(chunk-9): relay` is 8 markers in `daemon.rs`. The harness change is small (S2 already has `TempConfDir`, just spawn three; S3 needs a third TUN). The `net.py::test_tunnel_server` shape (foo→mid←bar, no direct foo↔bar `ConnectTo`) is the template.
