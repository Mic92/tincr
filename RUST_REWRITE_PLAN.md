...
| **6 — perf to 95%** | open | | The 23.4% gap remaining (1602 vs 2092 Mbps). Two body-sized copies left, both heap where C is stack: `daemon/net.rs:1718` 12-byte UDP-header wrap (the C author's own TODO at `net_packet.c:1027`: "if this copy turns out to be a performance concern, change sptps_send_record() to add some pre-padding"); `chapoly::open` receive-side (0.54% self + 0.64% memmove — same `seal_into` shape applies). `recvmmsg` (`net.rs:137`) untried. The 6 STUBs are the floor: `chunk-11-perf` ×3 (recvmmsg, conn-set tracking, `tcp_tunnel` alloc), `chunk-never` ×3. |
...
What's left for the 23.4% gap (≤5% is the bar):

- `daemon/net.rs:1718` — `Vec::with_capacity(12 + ct.len())` + extend
  for the UDP `[dst6‖src6‖ct]` wrap. The C `net_packet.c:1027` does
  `alloca` + memcpy and **has a TODO from 2013 about exactly this**
  ("change sptps_send_record() to add some pre-padding"). The C
  author knew; never had to act. The pre-pad fix would have
  `send_record_priv` reserve 12 bytes of head slack so the daemon
  writes `dst‖src` in-place — but that leaks daemon-layout knowledge
  through the SPTPS API. Alternatively: a daemon-field `[u8; 2048]`
  scratch (the C `vpn_packet_t` arena pattern).
- `chapoly::open` — receive side, 0.54% self + 0.64% memmove. Same
  `seal_into` shape: `open_into` decrypting in-place into a caller-
  reserved `&mut Vec`. The MAC-then-decrypt order means the buffer
  briefly holds ciphertext-then-plaintext; the C does this too.
- `recvmmsg` (`net.rs:137`) — untried. Per-packet syscall
  amortization on the receive side. The C doesn't do this either
  (one `recvfrom` per callback); a win if it works.

Buffer reuse, not `bytes::BytesMut` — the API shape (SPTPS in/out is
`&[u8]`) doesn't need refcounted slices.
