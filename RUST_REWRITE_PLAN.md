...
| **6 chunk 12+ — residuals** | open | | (residuals sweep — in progress.) What's left at 17 stubs. `chunk-9-relay` (6): TTL `getsockname` — only matters for TIME_EXCEEDED-from-relay-hop. `chunk-11-perf` (3): the 18% gap — per-packet `Vec` reuse (the C arena pattern), `recvmmsg`. `chunk-never` (3): `try_harder` brute-decrypt, legacy crypto. `chunk-12-overwrite-mac` (2), `chunk-12` maxoutbufsize (2), `chunk-9b` stray (1). |
...
| `route.c` | 1176 | ~95% | Chunk 7: `route_ipv4`. Chunk 9: v6/ICMP/MSS/ARP/NDP/TTL. **C-is-WRONG #8** (`:344`). `route_mac` (`52f6f348`) + `learn_mac`/`age_subnets` (`bc9f223b`) WIRED in `2bbd51b0`. Full RMODE_SWITCH dispatch (`:1159`). `route_broadcast` (`:559-565`). FMODE_KERNEL (`:1135-1138`). Left: FRAG_NEEDED (in progress). `fragment_ipv4` + `overwrite_mac` snatching (`:830,972`): NOT-PORTING. |
