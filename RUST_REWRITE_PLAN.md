...
| `control.c` | 241 | ~20% | REQ_STOP + REQ_DUMP_CONNECTIONS. 12/14 `REQ_*` left (chunk 8). CLI client side already speaks the protocol; daemon side is `match` arms that walk trees and `writeln!`. `init_control` landed in chunk 2. |
...
| `node.c` | 232 | ~10% | `Daemon::nodes: HashMap<String, NodeState>` stub. `lookup_node`/`node_add` are the HashMap ops. `tinc-graph::Node` is name+edges; `NodeState` adds RUNTIME state (which conn, edge addr/weight/options). Chunk 5: node_id_t for UDP, per-tunnel SPTPS, `dump_nodes`. |
