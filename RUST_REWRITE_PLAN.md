...
| `tincd.c` | 735 | ~40% | `main.rs` argv + signal install + SIGHUP→reload (chunk 10). `-o KEY=VALUE` + `-n NETNAME` flags (`764674b8`). Left: `parse_options` rest (`-d`/`-L`/`-s`/`--logfile`), `drop_privs`, `umbilical`, `ProcessPriority`, `mlock`, `daemon()`. |
