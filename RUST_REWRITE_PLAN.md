...
| **6 chunk 12+ — residuals** | open | | What's left at 30 stubs. `chunk-9-relay` (6): TTL `getsockname` — only matters for TIME_EXCEEDED-from-relay-hop. `chunk-12-strictsubnets` (5): hosts/-file-authorization gate that predates tunnelserver; needs `load_all_nodes` Subnet= parsing. `chunk-11-perf` (4): the 18% gap — per-packet `Vec` reuse (the C arena pattern), `recvmmsg`. `chunk-never` (3): `try_harder` brute-decrypt, legacy crypto. `chunk-12-{bind,overwrite-mac}` singles (4). `chunk-9b/9c/9-lzo-hi` strays (4). `chunk-12-tcp-fallback` (1) is the PACKET 17 receive routing (vs the 21 binary path which `300a8e96` fixed). `chunk-12` maxoutbufsize (1). |
...
| `net_packet.c` | 1938 | ~85% | **The hot path.** Chunk 7: send/recv core. Chunk 9: PMTU/compression/`try_tx` chain. `send_sptps_data` relay decision tree (`:965-1056`). All three chunk-12 leaves WIRED: `choose_local_address`/`adapt_socket` (`67e0dc22`), `broadcast_packet` target selection (`2bbd51b0`), `receive_tcppacket_sptps` ladder (`300a8e96` — the architectural-trap fix). Send/receive offset switch-aware (`:696-700`, `:1108`). Left: `receive_tcppacket` (PACKET 17 routing, `chunk-12-tcp-fallback`), `try_harder` (chunk-never), legacy crypto (`:800-960`, chunk-never). |
| `route.c` | 1176 | ~95% | Chunk 7: `route_ipv4`. Chunk 9: v6/ICMP/MSS/ARP/NDP/TTL. **C-is-WRONG #8** (`:344`). `route_mac` (`52f6f348`) + `learn_mac`/`age_subnets` (`bc9f223b`) WIRED in `2bbd51b0`. Full RMODE_SWITCH dispatch (`:1159`). `route_broadcast` (`:559-565`). FMODE_KERNEL (`:1135-1138`). Left: `fragment_ipv4` (niche), `overwrite_mac` snatching (`:830,972` — router-on-TAP only). |
...
**Post-chunk-12-switch**: 19 of 35 covered (`ns_ping`, `device_fd`, `security`, `splice`, `compression`, `basic`, `sptps_basic`, `net.py::tunnel_server`, `invite/join` ×3, `scripts`, `proxy` (full), `address_cache`, `device_tap` + 2 partial). 9 CLI-only (tinc-tools). 4 deliberately-never (`legacy_protocol`, `algorithms`, `sandbox`, `device_multicast`). 3 remaining gaps: `bind_address.py`/`bind_port.py` (`chunk-12-bind`), `systemd.py` (LISTEN_FDS). `device_raw_socket.py` no demand.
