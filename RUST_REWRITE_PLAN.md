...
| **N coverage-gap fixes** | ✅ 9+5 commits | `tincd: wire Forwarding=off gate in dispatch_route_result` | The audit (`bcc5c3e3`) applied the IP_MTU lesson systematically and found 31 gaps including 4 HIGH. All 4 HIGH ported: `f58ebde4` IP_MTU_DISCOVER (kernel cache never populated), `63146c64` KeyExpire (nonce-reuse at ~9h sustained throughput; **C-nolegacy has the same bug**), `727febdb` broadcast subnets (mDNS/DHCP/NDP got ICMP-unreachable; sentinel-`""` not Option to avoid file-conflict ripple), `e3513e52` Forwarding=off (parsed-but-ignored; **simple 3-node relay doesn't hit the gate** — needed StrictSubnets to force decrypt-at-relay). 5 MED/LOW: `5ceb8011`+`170e8f4a` complete the myself->options chain, `ea98247d` edge-walk tier-2, `764674b8` -o/-n flags, `26a513d4` MSG_NOSIGNAL. **Wave 2 (5 more)**: `d58de33d` Name=$HOST/$ENV (sanitize-then-check_id; only the env-expanded branch sanitizes), `4b463f5a` SO_RCVBUF/SNDBUF + SO_BINDTODEVICE + SO_MARK (**BindToInterface is a HARD fail unlike the others** — silently binding to the wrong interface defeats the security intent; the `actual<size` not `!=` check is because Linux doubles internally), `17ca9397` REQ_DUMP_TRAFFIC (**gap row was stale** — counters/increments already existed at `tunnel.rs:85-88`/`daemon/net.rs:567,736,815`; only the dump arm was missing), `3002adfb` lazy getaddrinfo (resolve at next_addr cursor not at open; `reset()` ≡ C `freeaddrinfo` — fresh resolve each retry round, dynamic DNS picks up new IP; still blocking, like C), `b2e8774c` sd_notify (no libsystemd — protocol is a Unix dgram with KEY=value text; would have been the first C dep). **Wave 3 (4 more)**: `da802250` bind_reusing_port + add_listen_address (`Port=0` → same TCP/UDP port; the C `from_fd` dance collapses to "ALWAYS try the TCP port we just got"), `e0b62259` purge + REQ_PURGE (two-pass node GC; **integration test exposed Rust-is-WRONG #6** — `broadcast_line` queues to outbuf without arming WRITE), `374bdfa6` detach/-D + drop_privs/-U/-R + mlockall/-L + ProcessPriority + -d/--logfile (the `tincd.c:640-720` ordering: drop_privs MUST come after setup_network because tinc-up needs root; **detach default broke 33 test callsites**, new `tincd_cmd()` bakes in `-D`; -s syslog left as warn-unimplemented), `81c27237` PriorityInheritance (**C-nolegacy never had it on the SPTPS path** — `:920-946` is in `send_udppacket`; we port the feature where C didn't reach). 31 → ~5 gaps remaining. |
| **N idiom audit + refactors** | ✅ 6 commits | `proto: bitflags! ConnOptions — type-safe over bare u32` | Code-audit pass (`/home/joerg/.claude/outputs/{rust-idiom,crate-gap}-findings.md`), net +27 LOC. Not coverage — invariants moved into types. **`4f99cd3a` zerocopy** (−180 LOC): 8 `#[repr(C, packed)]` structs had hand-written `to_bytes`/`from_bytes` repeating `b[2..4].copy_from_slice(&self.f.to_ne_bytes())` 41 times; the serialization was a manual transmute. `FromBytes`/`IntoBytes` derives compile-check the same no-padding invariant the `const _: ()` asserts already proved. **`7f512b85` `mtuprobes: i32` → `enum PmtuPhase`**: the single int encoded a 5-phase state machine via sign+magnitude (`0..19`=discovery, `20`=fix, `-1`=steady, `-2..-3`=revalidate, `-4`=lost); every gate was a hand-written range check; the off-by-one risks at `< -3`/`== -1`/`== 20` became compiler-checked match arms. **`268146a8` `bitflags! ConnOptions`**: `from_bits_retain` not `_truncate` — the C accepts unknown bits, wire compat; `prot_minor()`/`with_minor()` for the top-byte overload (txpath/udp_info gates left as `.bits() &` for now — bitflags-finish agent owns those files). **`66bafe30` `UdpInfoAction` carries `NodeId`**: 8 caller `.expect("X implies from exists")` migrated INTO `on_receive_udp_info` where the invariant is locally provable; same shape applied to `MtuInfoAction`. **`1faf1db8`** `udp_ping_rtt: i32` → `Option<u32>` (`-1` sentinel; wire format keeps `-1` via `map_or` at the dump site only). **`e5bec96d`** three parallel Vecs (`listeners`/`listener_udp_io`/`listener_tos`) → `Vec<ListenerSlot>` (the `last_tos` one was tactical merge-avoidance from `81c27237`, that agent has landed). |
...
| `meta.c` | 322 | ~85% | `feed`/`feed_sptps` + `dispatch_sptps_outputs`. `Record` arm → `record_body` strip + `check_gate`; `HandshakeDone` arm → `send_ack`. `send()` grew the `sptps_send_record` branch. Left: `tcplen` arm (`:143-152`, `tcppacket_h` body delivery, chunk 8). **`meta.c:98` `safe_write()` is push-based** — every C `send_request` writes synchronously; we are queue-then-arm-WRITE; see Rust-is-WRONG #6. |
...
| `control.c` | 241 | ~75% | 10/12 `REQ_*` arms (STOP, RELOAD, DUMP_{NODES,EDGES,SUBNETS,CONNECTIONS,TRAFFIC}, RETRY+DISCONNECT `14cf3ce4`+`17ca9397`, PURGE `e0b62259`). CLI client side speaks the full protocol; daemon side is `match` arms. `init_control` landed in chunk 2. Missing arms fall through to `REQ_INVALID` (matches C's default). Left: PCAP, LOG. |
| `pidfile.c` | tiny | ✅ | `Pidfile::read` (CLI side) + write (daemon, chunk 2). |
| `net_socket.c` | 884 | ~90% | `tincd::listen` (listeners + tarpit) + `tincd::outgoing` (`do_outgoing_connection`, `try_outgoing_connections`, `retry_outgoing`, `finish_connecting`, the `handle_meta_io` connecting probe). All proxy modes wired (`daemon.rs:1191` comment is stale). `IP_MTU_DISCOVER` (`f58ebde4` — kernel never populated the PMTU cache that `05ba1f82` reads). `MSG_NOSIGNAL` on `conn.rs` send (`26a513d4` — defense-in-depth; Rust runtime SIG_IGNs SIGPIPE pre-main). `SO_RCVBUF`/`SO_SNDBUF` + `SO_BINDTODEVICE` + `SO_MARK` on listeners (`4b463f5a` — BindToInterface is the one hard-fail; the buf-clamp warning only fires when explicitly configured, the 1MB default getting clamped on every boot would be log noise). Left: `IP_TOS`/`IPV6_TCLASS` (`configure_tcp:93-100`), `SO_MARK` on outgoing TCP (`:383`). |
| `net_setup.c` | 1336 | ~75% | `setup_myself` skeleton + `setup_myself_reloadable` (chunk 10), `load_all_nodes` (chunk 11), `device_enable`/`device_disable` script hooks (`daemon.rs:1667,1893`). KeyExpire timer armed (`63146c64`). Global `IndirectData`/`TCPOnly`/`PMTUDiscovery`/`ClampMSS` parsed (`170e8f4a`). `DEFAULT_BROADCAST_SUBNETS` inserted at setup (`727febdb`). `Name = $HOST`/`$ENV` expand (`d58de33d`). `bind_reusing_port` + `add_listen_address` BindToAddress walk (`da802250` — with `Port=0` kernel picks DIFFERENT ephemeral ports for TCP/UDP; steal the TCP port for UDP; fallback to fresh port if taken). Left: remaining misc config keys. |
| `tincd.c` | 735 | ~55% | `main.rs` argv + signal install + SIGHUP→reload (chunk 10). `-o KEY=VALUE` + `-n NETNAME` flags (`764674b8`). `sd_notify` module exists (`b2e8774c` — no libsystemd; main.rs wiring is on sd-notify-wire agent's plate). `detach()`/`-D` + `drop_privs`/`-U`/`-R` + `mlockall`/`-L` + `ProcessPriority` + `-d`/`--logfile` (`374bdfa6` — **C ordering is load-bearing**: detach → mlockall → setup_network → ProcessPriority → drop_privs; tinc-up needs root so drop_privs MUST come after setup; detach default broke every test that spawns the daemon, `tincd_cmd()` helper bakes in `-D` at all 33 sites). Left: `-s` syslog (warn-unimplemented), `umbilical`. |
...
| `net.c` | 527 | ~95% | `timeout_handler` ping sweep + laptop-suspend (chunk 8), `periodic_handler` storm-detect (chunk 8), `reload_configuration` SIGHUP mark-sweep (chunk 10 — `reload.rs::diff_subnets/conns_to_terminate`). `retry()` (`14cf3ce4` — SIGALRM zeros backoff + kicks ping sweep; `Instant` has no zero so `checked_sub(pingtimeout+1)` for the same effect). `purge()` (`e0b62259` — two-pass node GC; **the C `:83` early-return is subtle**: a single incoming edge to a single unreachable node aborts ALL pass-2 deletions — only fires when a REACHABLE node still claims an edge to a dead one, gossip hasn't caught up, wait for next purge; the integration test exposed Rust-is-WRONG #6). The mark-sweep is `BTreeSet::difference`; the C's `expires=1` flag is a splay-tree workaround. Left: nothing structural. |
...
| `route.c` `inet_checksum` + headers | ~100 | ✅ | `packet.rs`. `#[repr(C, packed)]` Ipv4Hdr/Ip6Hdr/IcmpHdr/Icmp6Hdr/EtherArp + KAT-locked checksum (native-endian `memcpy` load, RFC 1071 §2(B)). zerocopy derives (`4f99cd3a` — −180 LOC manual `b[2..4].copy_from_slice(&self.f.to_ne_bytes())` boilerplate; structs already satisfied the invariants, the `const _: () = assert!(size_of == N)` checks already proved no padding). |
| `process.c` | 243 | ~90% | `detach()` ported in `374bdfa6` (lives in `main.rs` not here — the C splits across `process.c:200-243` + `tincd.c:640`). Ship-#1's SIGPIPE-from-dropped-stderr finding applies to script spawn. Left: nothing structural; `script.rs` covers the rest. |
