...
| 5 chunk 4b — `send_ack`/`ack_h`, world-model stub | | | **The chunk-4a terminate goes away.** `send_ack` (`protocol_auth.c:826-868`, 43 LOC) queues `"%d %s %d %x"` (port, weight, options-bitfield); `ack_h` (`:948-1066`, 119 LOC) parses the peer's, calls `node_add` + `edge_add` + `graph()` + `send_everything`. **Can't port `ack_h` without somewhere to put the node** — it does `n = lookup_node(hisname); if(!n) { n = new_node(); node_add(n); }`. So this chunk MUST include enough of the world model to hold one peer. **The (b) path**: a `Daemon::nodes: HashMap<String, NodeState>` where `NodeState` is just `{ id: NodeId, conn: Option<ConnId> }` — `tinc-graph::Graph` already has `add_node`/`add_edge`; the daemon-side `NodeState` is the *runtime* annotation (which connection, what UDP addr, the per-tunnel SPTPS) that `tinc-graph::Node` doesn't carry. `send_everything` iterates empty trees → sends nothing. Two daemons can ACK each other; no subnets propagate. **The integration test extends `peer_handshake_reaches_done`**: after the test-side completes its handshake, send the test's ACK, recv the daemon's ACK, parse it (tinc-proto already has the format), assert weight/options. The connection STAYS UP. Then `tinc dump connections` against the daemon (control socket) → one row. **`Output::Record` arm wires up** (`meta.c:153-161`): post-handshake records go through `receive_request` dispatch — same as the pre-SPTPS line dispatch, but now the bytes come out of SPTPS not the LineBuf. The `match` arm calls the same `check_gate` + handler the cleartext path does. ~400 C LOC traced → ~2k Rust file-LOC at the post-cleanup 5× ratio. |
| 5 chunk 5 — world model proper: subnet trees, edge propagation | | | `node.c` (232) + `edge.c` (138) + `subnet.c` (409) + `protocol_edge.c` (322) + `protocol_subnet.c` (261) + `protocol.c::seen_request` (~80). **The `subnet_tree` is the routing table.** `subnet_compare_ipv4` (`subnet_parse.c:137`) sorts by `prefixlen-DESC, addr, weight, owner` — `b->prefixlength - a->prefixlength` is descending so in-order iteration finds longest match first. `lookup_subnet_ipv4` (`subnet.c:256`) is a *linear scan* through that order with a `maskcmp` (already in tinc-proto) per entry, plus a hash-cache for the hot path. Port the linear scan + cache; **don't** reach for a prefix-trie crate — the C does `splay_each` linear, the iperf3 benchmark gate (already in test strategy) catches if it's too slow. `BTreeMap<SubnetKey, OwnerNodeId>` where `SubnetKey: Ord` matches the C compare (no subtraction trick — `i32` overflow). **`seen_request` is anti-loop**: ADD_EDGE/ADD_SUBNET messages flood the mesh; without dedup, a cycle bounces forever. `protocol.c:78-228` is a TTL'd cache (`past_request_tree` + `age_past_requests` timer). The key is the *full message string* (`strcmp` compare). `HashMap<String, Instant>` + a periodic sweep timer. `protocol_edge.c::add_edge_h` and `protocol_subnet.c::add_subnet_h` are the message handlers — both call `seen_request(request)` first, both call `graph()` after. **`graph()` glue**: the C `graph.c::graph()` (`:327`) does `sssp()` then walks the result to set `n->status.reachable` and fire `subnet_update()` for the became-reachable/-unreachable transitions. `tinc-graph::sssp` returns `Vec<Option<Route>>`; the daemon walks it, diffs against the previous reachability, calls `subnet_update` (script hooks, deferred) for transitions. The `mst()` half sets `connection_t::status.mst` for the broadcast tree; deferred (no broadcast yet). ~1440 C LOC → ~7k Rust file-LOC. **The chunk-5 integration test still has test-process-as-initiator** (no outgoing connections until chunk 6): the test pump sends a fake ADD_SUBNET line, asserts the daemon's `dump subnets` reflects it; then asserts the daemon SENT its own ADD_SUBNET (recv on the test side, parse with tinc-proto). The two-real-daemons test is chunk 6's prize. |
| 5 chunk 6 — outgoing connections | | | Today we only ACCEPT. `do_outgoing_connection` (`net_socket.c:564-662`, ~100), `setup_outgoing_connection` (`:664-732`), `try_outgoing_connections` (`:815-884`), `retry_outgoing` (`:405-417`, exponential backoff timer), `address_cache.c` (284). **The async-connect dance**: nonblocking `connect()` returns `EINPROGRESS`; you `epoll` for WRITE; when writable, `getsockopt(SO_ERROR)` tells you if it actually connected. socket2 has `Socket::connect` that does this; the WRITE-readiness path is a new `IoWhat::OutgoingPending(ConnId)` variant. `address_cache.c` is `getaddrinfo` results per-hostname with a try-order shuffle; can be `Vec<SocketAddr>` per-node and a cursor. **The chunk-5 integration test gets simpler**: instead of test-process-is-the-initiator, BOTH sides are real daemons with `ConnectTo = peer` in tinc.conf. ~550 C LOC → ~2.5k Rust. |
...
| `connection.c` | 175 | ~80% | `tincd::conn::Connection`. Fields landed; `dump_connections` lands with chunk-8 control protocol. |
| `meta.c` | 322 | ~60% | `tincd::conn::feed`/`feed_sptps`. Left: full handler dispatch in `Output::Record` arm (chunk 4b). |
| `protocol.c` | 245 | ~60% | `tincd::proto::check_gate` (the dispatch). Left: `seen_request` anti-loop cache (chunk 5). |
| `protocol_auth.c` | 1066 | ~25% | `id_h` peer+control branches done. Left: `send_ack` (43) + `ack_h` (119) chunk 4b; `?` invitation branch (~80) chunk 10; legacy (~400) defer. `send_everything` (`:870-900`, 30) needs the world-model trees to iterate. |
| `keys.c` | 334 | ✅ | `tincd::keys`. The `& ~0100700u` perm-check bug ported as C-is-WRONG #7. |
| `control.c` | 241 | ~15% | REQ_STOP only. 13/14 `REQ_*` left (chunk 8). CLI client side (`ctl.rs`) already speaks the protocol; daemon side is `match` arms that walk trees and `writeln!`. `init_control` landed in chunk 2. |
...
| `node.c` | 232 | 0 | chunk 4b/5. `tinc-graph::Node` is name+edges; `node.c` adds RUNTIME state (node_id_t for UDP, per-tunnel SPTPS, last-seen). The split: `tinc-graph::Graph` for topology, `Daemon::nodes: HashMap<NodeId, NodeState>` for runtime. |
| `edge.c` | 138 | 0 | chunk 5. `tinc-graph::add_edge` exists; `edge.c` is the per-node edge tree + `dump_edges`. |
| `subnet.c` | 409 | 0 | chunk 5. `tinc-proto::Subnet` is wire format; `subnet.c` is the *lookup tree* (routing table). `subnet_compare_ipv4` sorts `prefixlen-DESC, addr, weight, owner` (`subnet_parse.c:140` is `b - a`); in-order iteration finds longest match. Linear scan + hash cache. Port faithful; benchmark gate catches if too slow. |
| `protocol_edge.c` + `protocol_subnet.c` | 583 | 0 | chunk 5. Handlers for `ADD_EDGE`/`DEL_EDGE`/`ADD_SUBNET`/`DEL_SUBNET`. `tinc-proto::msg::{edge,subnet}` already has the wire format; these are `seen_request` check + tree mutation + `graph()` call. |
| `graph.c` | 327 | algo ✅ | `tinc-graph::sssp`/`mst`. Left: the `graph()` *glue* (~100) that walks `sssp` result, diffs reachability, fires `subnet_update` for transitions. |
...
| `address_cache.c` | 284 | 0 | chunk 6. `getaddrinfo` results cached per-hostname. `Vec<SocketAddr>` + a cursor. |
