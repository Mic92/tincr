//! `MacosFast` — batch UDP send via `sendmsg_x(2)`.
//!
...
// `sendmsg_x` is not in libc/nix; raw FFI is unavoidable. Scoped to
// the call site below; the rest of egress stays `forbid(unsafe_code)`.
#![allow(unsafe_code)]
...
/// the xnu source. `repr(C)` so field order/padding is the C ABI.
/// Field names mirror the C struct verbatim for greppability.
#[repr(C)]
...
unsafe impl Send for MacosFast {}
...
pub(crate) struct MacosFast {
    /// Per-frame `sendto` fallback. Same dup as `Portable` always had.
    fallback: Portable,
...
impl MacosFast {
    ...
    /// the connected dup is created lazily on first batch (we don't
    /// know the peer address at setup time).
    ///
    /// # Errors
    /// `io::Error` from `dup(2)`.
    pub(crate) fn new(udp: &Socket) -> io::Result<Self> {
        ...
        // each `sendmsg_x` call. Boxed via `Box::new` of a zeroed
        // array would put ~6 KiB on the stack first; `alloc_zeroed`
        // is what we want but `Box::<[T; N]>::new_zeroed()` is
        // unstable, so go via `vec!`→boxed-slice→try_into like
        // `UdpRxBatch` does.
...
impl UdpEgress for MacosFast {
    fn send_batch(&mut self, b: &EgressBatch<'_>) -> io::Result<()> {
        // Single frame: no syscall win, and `count == 1` is the
        // immediate-send / cold path where the `connect()` setup
        // would be pure overhead. Same wire bytes either way.
...
mod tests {
    ...
    /// Wire-equivalence: `MacosFast::send_batch` produces the same
    ...
    fn macos_fast_splits_at_stride() {
        ...
        let mut p = MacosFast::new(&tx).unwrap();

        // 3 chunks of 10, last short at 7.
        ...
        // Second batch to same dst reuses the connected dup (the
        // `ensure_connected` fast path). Different bytes prove it's
        // not stale data.
    ...
    /// `count == 1` goes through the portable fallback (no connect,
    /// no `sendmsg_x`). Proves the immediate-send path still works.
    #[test]
