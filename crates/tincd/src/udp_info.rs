...
impl PmtuSnapshot {
    /// Largest size confirmed to fit over UDP; `None` = nothing
    /// confirmed yet or UDP dead (#21). Does not require convergence
    /// so `MTU_INFO` stays clamped during rediscovery.
    #[must_use]
...
mod tests {
    ...
    fn mtu_adjust_table() {
        ...
        let fresh = Some(PmtuSnapshot { minmtu: 0, maxmtu: 1500 }); // nothing confirmed
