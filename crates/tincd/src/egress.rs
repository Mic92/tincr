...
// needs a scoped `#[expect(unsafe_code)]` on its module.
#![deny(unsafe_code)]
...
impl Portable {
    ...
    /// Borrow the dup'd socket. `macos::Fast` calls `as_raw_fd()` on
    /// it for the `sendmsg_x` syscall; otherwise the field stays
    /// encapsulated for `Portable`'s own send loop.
    #[cfg(target_os = "macos")]
