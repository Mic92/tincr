...
mod tests {
    ...
    /// Write IP bytes to a pipe, `FdTun` reads at +14, synthesizes
    /// the ethertype at +12, returns len+14. Only byte 0 of the
    /// packet matters (the version nibble); the rest is opaque payload.
    #[test]
    fn read_ip_via_pipe() {
